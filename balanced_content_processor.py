#!/usr/bin/env python3
"""
Balanced Content Processor
- Keep more meaningful content while maintaining quality
- Ensure word clouds have sufficient content
- Balance between filtering and content richness
- Process all 3 banks for optimal content balance
"""
...
class BalancedContentProcessor:
    def __init__(self, deepseek_api_key: str, num_workers: int = 15):
        ...
        # Only fix the most problematic phrases, keep more content
        self.phrase_fixes = {
        ...
        # Only remove the most problematic phrases
        self.remove_phrases = {
    ...
    def balanced_filtering(self, phrases: Dict[str, int], sentiment: str) -> Dict[str, int]:
        """Balanced filtering that keeps more meaningful content."""
        filtered_phrases = {}
        
        print("Balanced filtering to keep more meaningful content...")
        ...
        logger.info(f"Balanced filtered {len(phrases)} phrases to {len(filtered_phrases)} phrases")
    ...
    def ai_balanced_merging(self, phrases: Dict[str, int], sentiment: str) -> Dict[str, int]:
        """Use AI for balanced merging to keep more content."""
        if len(phrases) <= 1:
            return phrases
        ...
        # Take more phrases to keep more content
        top_phrases = sorted_phrases[:500]  # Keep more phrases
        ...
        def process_merging_batch(batch):
            ...
            # Create prompt for balanced AI merging
            phrase_list = [f"'{phrase}' (frequency: {freq})" for phrase, freq in batch]
            phrases_text = "\n".join(phrase_list)
            
            prompt = f"""Analyze these {sentiment} sentiment phrases about banking apps and create meaningful, clear phrases while keeping more content.
            
            Rules:
            1. Merge only very similar phrases into the most representative one
            2. Keep more content - don't be too aggressive in merging
            3. Fix any incomplete or unclear phrases
            4. Choose the most descriptive phrase for each group
            5. Focus on phrases that indicate specific problems or issues
            6. Reduce the number of phrases by only 20-30% (be conservative)
            7. Keep meaningful phrases even if they seem similar
            
            Phrases:
            {phrases_text}
            
            Return the result as a JSON array of objects with 'phrase' and 'frequency' fields.
            Keep more meaningful content while ensuring quality.
        ...
        print(f"AI balanced merging {len(top_phrases)} {sentiment} phrases with {self.num_workers} workers...")
    ...
    def process_bank_file(self, bank: str, sentiment: str) -> Dict[str, int]:
        """Process a single bank file with balanced content approach."""
        ...
        print(f"\nProcessing {filename} with balanced content approach...")
        ...
        # Round 1: Balanced filtering
        print("Round 1: Balanced filtering...")
        filtered_phrases = self.balanced_filtering(phrases, sentiment)
        
        # Round 2: AI balanced merging
        print("Round 2: AI balanced merging...")
        final_phrases = self.ai_balanced_merging(filtered_phrases, sentiment)
        
        # Save processed phrases to text file
        processed_filename = f"{bank}_{sentiment}_balanced_content_processed_phrases.txt"
    ...
    def save_processed_phrases_to_txt(self, phrases: Dict[str, int], filename: str):
        ...
        with open(filepath, 'w', encoding='utf-8') as f:
            f.write(f"Balanced Content - Processed Phrases\n")
            f.write("="*50 + "\n\n")
    ...
    def process_all_banks(self) -> Dict[str, Dict[str, Dict[str, int]]]:
        """Process all banks with balanced content approach."""
        banks = ['mox_bank', 'welab_bank', 'za_bank']
        sentiments = ['negative']  # Focus on negative as requested
        
        results = {}
        
        print(f"Processing all banks with balanced content approach...")
    ...
    def generate_wordcloud(self, phrases: Dict[str, int], sentiment: str, bank: str, filename: str):
        ...
        max_words = min(200, len(phrases))  # More words for richer content
        min_font_size = 12  # Smaller minimum for more content
        max_font_size = 140  # Large maximum for important phrases
    ...
    def generate_all_wordclouds(self, results: Dict[str, Dict[str, Dict[str, int]]]):
        ...
        print("\nGenerating balanced content word clouds...")
        
        for bank, sentiments in results.items():
            for sentiment, phrases in sentiments.items():
                if phrases:
                    filename = f"{OUTPUT_DIR}/{bank}_{sentiment}_balanced_content_wordcloud.png"
    ...
    def print_summary(self, results: Dict[str, Dict[str, Dict[str, int]]]):
        ...
        print("\n" + "="*60)
        print("BALANCED CONTENT PROCESSING SUMMARY")
        print("="*60)
        ...
        for bank, sentiments in results.items():
            ...
            for sentiment, phrases in sentiments.items():
                ...
                # Show top 15 phrases (more content)
...
def main():
    """Main function to run the balanced content processor."""
    ...
    print("Balanced Content Processor")
    print("="*50)
    print(f"Positive color: RGB{BalancedContentProcessor(api_key).positive_color}")
    print(f"Negative color: RGB{BalancedContentProcessor(api_key).negative_color}")
    print(f"Backing up existing wordclouds...")
    
    processor = BalancedContentProcessor(api_key, num_workers=15)
    
    # Backup existing wordclouds
    backup_dir = processor.backup_existing_wordclouds()
    
    print("Processing all banks with balanced content approach...")
    start_time = time.time()
    
    results = processor.process_all_banks()
    
    print("Generating balanced content word clouds...")
    ...
    print(f"\nBalanced content processing complete!")
